...
public class Game15Controller : MonoBehaviour 
{
	...
	{	
		...
		p_script = GetComponentsInChildren<Puzzle15> ();
		checkmas = new int[16];
...
//			LoadGame ();	
	...
	void CreatePuzzle()
	{
		...
		{		
			...
			for(int x = 0; x < 4; x++)
			{
				...
				{			
					...
					grid [x, y].name = "ID-" + i;
					checkmas [ii] = grid [x, y].GetComponent<Puzzle15> ().ID;
					grid [x, y].transform.parent = transform;
					i++;
					ii++;
				} 
		...
		foreach (BoxCollider2D box2d in boxes)
			box2d.enabled = true;
		foreach (Puzzle15 puz in p_script)
			puz.enabled = true;
	...
	static public void GameFinish()
	{
		...
		for(int y = 0; y < 4; y++)
		{
			for(int x = 0; x < 4; x++)
			{
				if(grid[x,y]) { if(grid[x,y].GetComponent<Puzzle15>().ID == i) i++; } else i--;
			}
		}
		...
		if(i == 15)
		{
			for(int y = 0; y < 4; y++)
			{
				for(int x = 0; x < 4; x++)
				{
					if(grid[x,y]) Destroy(grid[x,y].GetComponent<Puzzle15>());
				}
			}
